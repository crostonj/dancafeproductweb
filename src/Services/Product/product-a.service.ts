...
export class ProductAService {
  ...
  search(searchString: string){
    const url = this.ProductUrl + '?searchString=' + searchString;
    return this.http.get<InventoryItem[]>(url, this.httpOptions);
  }
  searchAdvanced(searchItem: InventoryItem){
    const url = this.ProductUrl + '?searchString=';
    return this.http.post<InventoryItem[]>(url, searchItem, this.httpOptions);
  }
